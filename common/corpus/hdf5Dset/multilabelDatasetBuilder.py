...
class MultilabelDatasetBuilder:
	...
	def buildTokenSamples(self, dset : MultilabelDataset, lblsForSpecialTkns : List[int], overlapRatio = 1/3):
		"""
		Builds the dataset with token level samples.

		:param dset: Empty dataset to fill in with samples. Must be in filling-in mode.
		...
		for ctIdx, ct in tqdm(enumerate(self.corpus.texts), "Building samples from corpus texts"):
			...
			for encIdx, enc in enumerate(encs):
				...
				tokenIds = np.array(inputTkns, np.int32)
				tokenEmbeds = inputTknEmbeds
				tokenMasks = np.array(inputTknsMask, np.int8)

				teRowIdx = dset.appendTextEncodingRow(tokenIds, tokenEmbeds, tokenMasks)
				...
				for tknIdx in range(numNonPadTkns):
					targetTokenLabels = np.zeros(len(self.corpus.labelDefs), np.int8)
					for tknLbl in tknLbls[tknIdx]:
						targetTokenLabels[tknLbl] = 1

					dset.appendSampleRow(
						textId=ctIdx, textEncodingIdx=teRowIdx, targetTokenIdx=tknIdx, targetTokenLabels=targetTokenLabels
					)
	
	def buildWordSamples(self, dset : MultilabelDataset, lblsForSpecialTkns : List[int], overlapRatio = 1/3) -> MultilabelDataset:
		"""
		...
		:param dset: Dataset to fill in with samples. Must be in filling-in mode.
		:param lblsForSpecialTkns: Labels to assign to special tokens.
		...
		for ctIdx, ct in tqdm(enumerate(self.corpus.texts), "Building samples from corpus texts"):
			...
			for wrdEmbeds, wrdMask, wrdOffsets in zip(seqWrdEmbedsAll, seqWrdMasksAll, seqWrdOffsetsAll):
				#build and register a text encoding row
				teRowIdx = dset.appendTextEncodingRow(None, wrdEmbeds, wrdMask)
				...
				for wrdIdx in range(numNonPadWrds):
					...
					for wrdLbl in wrdLbls[wrdIdx]:
						targetTokenLabels[wrdLbl] = 1

					dset.appendSampleRow(
						textId=ctIdx, textEncodingIdx=teRowIdx, targetTokenIdx=wrdIdx, targetTokenLabels=targetTokenLabels
					)
